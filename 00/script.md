...
「普通の参照はただ貸している意味しかありませんが, こちらは渡す側がもう値を使わないことを明示できる強みがあります
...
「rvalue 参照の注意点として, 戻り値型に rvalue 参照型を使ってはいけません
...
「あれ, 戻り値でもムーブしちゃっていいんじゃないんですか？なんか効率良くなりそうだしー

「それがですね, rvalue とはいえこれも参照なので実態はポインタなんですね
...
「ここで作った `Hoge` のインスタンスは, 関数を抜ける時に解放されます

「あっそっか, 無効な参照ができちゃうんだ

「はい, 普通の関数の戻り値で参照を返す意味は無いです

「じゃあメンバ関数はどうなの？

「いい目の付け所ですね. インスタンスが参照の場合はメンバの参照を返す意味があります
...
public:
  int &a() & { return _a; }
  int &&a() && { return std::move(_a); }
} hoge;
int &a = hoge.a();
int &&b = std::move(hoge).a();
```

「ほほー, メンバ関数の参照の修飾って rvalue のもあるんだね♪

「さて, 色々とムーブセマンティクスを見てきましたが, 原理的にはこれだけですね
...
「C++ と似た部分が多いので, ねねさんなら趣味でやっても楽しいと思いますよ
