...
Rust は式に重きを置いた言語なので, すべての記述が式になっている. そんな式のいくつかをみていこう.
...
括弧の中に式を書くもので, この式の結果は中の式の結果だよ. 俗に括弧式と言うね.
...
`..` 演算子を使えば簡単な数列のイテレータが作れるよ. これを使えば C の `for` の書き方とはおさらばできる.
